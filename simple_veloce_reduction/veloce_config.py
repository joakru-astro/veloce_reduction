...
class VelocePaths:
    ...
    def from_config(cls, config):
        ...
        if config['wave_dir'] != 'Default':
            paths.wave_dir = config['wave_dir']
        if config['trace_dir'] != 'Default':
            paths.trace_dir = config['trace_dir']
        # intermediate paths
        if config['blaze_dir'] != 'Default':
            paths.blaze_dir = config['blaze_dir']
        if config['master_dir'] != 'Default':
            paths.master_dir = config['master_dir']
    ...
    def __str__(self):
        ...
        message += f'Raw data directory: {self.raw_dir}\n'
        message += f'Extracted data directory: {self.extracted_dir}\n'
...
def validate_config(config):
    ...
    if config['reduce'] not in ['run', 'night', 'file']:
        raise ValueError('reduce must be "run", "night" or "file", not {config["reduce"]}')
    if config['calib_type'] not in ['SimLC', 'SimThXe', 'Interpolate', 'Static']:
        raise ValueError('calib_type must be "SimLC", "SimThXe", "Interpolate" or "Static", not {config["calib_type"]}')
    if config['arm'] not in ['all', 'red', 'green', 'blue']:
        raise ValueError('arm must be "all", "red", "green" or "blue", not {config["arm"]}')
    if config['amplifier_mode'] not in [2, 4]:
        raise ValueError('amplifier_mode must be 2 or 4, not {config["amplifier_mode"]}')
    if not isinstance(config['plot_diagnostic'], bool):
        raise ValueError('plot_diagnostic must be True or False, not {config["plot_diagnostic"]}')
    ...
    if not os.path.exists(config['raw_parent_dir']):
        raise FileNotFoundError(f'{config["raw_parent_dir"]} does not exist')
    ...
    if config['reduce'] == 'night' and not os.path.exists(os.path.join(config['raw_parent_dir'], config['run'], config['date'])):
        raise FileNotFoundError(f'{config["date"]} does not exist in {config["raw_parent_dir"]}/{config["run"]}')
    ...
    if not os.path.exists(config['extracted_parent_dir']):
        raise FileNotFoundError(f'{config["extracted_parent_dir"]} does not exist')
    # validate internal paths
    if config['wave_dir'] != 'Default' and not os.path.exists(config['wave_dir']):
        raise FileNotFoundError(f'{config["wave_dir"]} does not exist')
    if config['trace_dir'] != 'Default' and not os.path.exists(config['trace_dir']):
        raise FileNotFoundError(f'{config["trace_dir"]} does not exist')
